...
const generateOccursPDF = async () => {
    ...
    doc.text(`Activity name: ${state.occurrence.name}`, 20, 40);
    doc.text(`Date: ${state.occurrence.dateActivity.toISOString().substring(0, 10)}`, 20, 50);
    doc.text(`Price : ${state.occurrence.price} CHF`, 20, 70);
    doc.text(`List of participants :`, 20, 90);
    ...
    doc.text(`Total participants : ${state.participant_list.length}`, 20, 100 + state.participant_list.length * 10);
    doc.text(`Total price : ${state.participant_list.length * state.occurrence.price} CHF`, 20, 110 + state.participant_list.length * 10);
    doc.save(`participants_${state.occurrence.name}.pdf`);
};

const generateOccursCSV = async () => {
    state.participant_list = await getParticipantsActivity(state.occurrence.id, state.occurrence.dateActivity);
    let csvContent = "data:text/csv;charset=utf-8," + "Name,Date,Time,Lastname,Firstname,Email,Price\n";
    ...
    csvContent += `Total participants : ${state.participant_list.length},Total price : ${state.participant_list.length * state.occurrence.price}`;
