...
JNIEXPORT jobject JNICALL Java_ShowWinInfo_getWinInfo(JNIEnv *env, jobject obj)
{
  ...
  if ( osvi.dwMajorVersion >= 6 )
  {
    ...
    wininfo_build = osvi.dwBuildNumber;

    jclass cls = (*env)->FindClass(env, "WinInfo");
    if (cls == NULL) return NULL; // exception thrown
    jmethodID mid = (*env)->GetMethodID(env, cls, "<init>", "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V");
    if (mid == NULL) return NULL; // exception thrown

    jstring wintype = (*env)->NewStringUTF(env, wininfo_type);
    jstring wined = (*env)->NewStringUTF(env, wininfo_edition);
    jstring winsp = (*env)->NewStringUTF(env, wininfo_sp);

    jobject result = (*env)->NewObject(env, cls, mid, wintype, wined, winsp);

    jfieldID fieldBuild = (*env)->GetFieldID(env, cls, "build", "I");
    (*env)->SetIntField(env, result, fieldBuild, wininfo_build);
    jfieldID fieldArch = (*env)->GetFieldID(env, cls, "arch", "B");
    (*env)->SetByteField(env, result, fieldArch, wininfo_arch);
