...
public class BTreeItr<E> {
    ...
    public BTreeItr<E> up() {
        ...
        r.down = up;
        r.up = up.parent;
        r.isLeftArc = (r.up != null && r.up.left == r.down);
        return r;
    }
     ...
     * PRE: left child exist
     */
    public void rotateLeft() {
        if (!hasRight()) return;

        BTree<E> b = right().left().cut();
        BTree<E> y = right().cut();
        BTree<E> x = cut();
        ...
        /*
        //Store the actual right child
        BTreeItr<E> oldRightBranchItr = new BTreeItr<>(right().cut());

        //Cut the left child of the right child
        //Then paste them in the right child
        right().paste(oldRightBranchItr.left().cut());

        //Paste the tree in the left branch
        oldRightBranchItr.left().paste(cut());

        //update itr
        if(isRoot()) whole = oldRightBranchItr.whole;
        paste(oldRightBranchItr.cut());*/
    }

    /**
     * Moves the neighboring nodes according to a right rotation.
     * PRE: right child exist
     */
    public void rotateRight() {
        //Check if the right child exist
        if (!hasLeft()) return;

        /*BTree<E> b = left().right().cut();
        BTree<E> x = left().cut();
        BTree<E> y = cut();
        ...
        //Store the actual right child
        BTreeItr<E> oldLeftBranchItr = new BTreeItr<>(left().cut());

        //Cut the left child of the right child
        //Then paste them in the right child
        left().paste(oldLeftBranchItr.right().cut());

        //Paste the tree in the left branch
        oldLeftBranchItr.right().paste(cut());

        //update itr
        if (isRoot()) whole = oldLeftBranchItr.whole;
        paste(oldLeftBranchItr.cut());
