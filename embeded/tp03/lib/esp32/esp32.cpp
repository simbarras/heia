...
bool Esp32::startBlePublisher(bool autoAdvertising)
{
    if (initBlePublisher())
...
bool Esp32::initBlePublisher()
{
    smutex_.lock();
    if (espParser_.recv("ready") && espParser_.send("AT+BLEINIT=2") && espParser_.recv("OK") &&
        espParser_.send("AT+BLENAME=\"%s\"", kPublisherName) && espParser_.recv("OK") &&
...
void Esp32::callbackPublisherNotify()
{
    tr_info("Notify reveived");
    publisherNotify();
    if (callbackPublisherNotify_ != NULL) this->callbackPublisherNotify_();
}
...
bool Esp32::publisherNotify()
{
    ...
    tr_info("Temperature: %3.2f C", temperature, 0xb0);
    tr_info("Humidity:    %2.2f %%", humidity, 0x25);
    tr_info("Pressure:    %4.2f mbar's", pressure);
...
void Esp32::parsePublisherCallback()
{
    callbackPublisherActivated_ = true;
    while (true) {
        parsePublisherCallbackSem_.acquire();
...
bool Esp32::initBleSubscriber()
{
    smutex_.lock();
    // TODO: A ameliorer (Connection dynamique)
    char* mac = "EC:94:CB:7D:E4:F2";
    tr_info("Try to connect to BLE device %s", kPublisherName);
    if (espParser_.recv("ready") && espParser_.send("AT+BLEINIT=1") && espParser_.recv("OK") &&
        espParser_.send("AT+BLENAME=\"%s\"", kSubscriberName) && espParser_.recv("OK") &&
        espParser_.send("AT+BLEGATTSSRVCRE") && espParser_.recv("OK") &&
        espParser_.send("AT+BLEGATTSSRVSTART") && espParser_.recv("OK") &&
        espParser_.send("AT+BLECONN=0,\"%s\"", mac) && espParser_.recv("OK")) {
        ...
        tr_info("BLE connected");
        return true;
    } else {
        tr_error("Fail to connect to BLE device %s", kPublisherName);
        smutex_.unlock();
        return false;
    }
...
bool Esp32::getIPAddress(uint8_t* ip, uint8_t* gw, uint8_t* msk)
{
    ...
    if (espParser_.recv("+CIPSTA:ip:\"%hhd.%hhd.%hhd.%hhd\"\r\n", &ip[0], &ip[1], &ip[2], &ip[3]) &&
        espParser_.recv(
...
{
    smutex_.lock();
    espParser_.send("AT+MQTTPUB=0,\"%s\",\"%s\",%d,%d", topic, data, qos, retain);
    if (espParser_.recv("OK")) {
        smutex_.unlock();
        return true;
    } else {
...
bool Esp32::mqttConnectBroker(const char* host, int port, const char* id, const char* user, const char* pwd)
{
    smutex_.lock();
    setATDebug(true);
    espParser_.send("AT+MQTTUSERCFG=0,1,\"%s\",\"%s\",\"%s\",0,0,\"\"", id, user, pwd);
    espParser_.recv("OK");
    espParser_.send("AT+MQTTCONN=0,\"%s\",%d,1", host, port);
    if (espParser_.recv("OK")) {
        setATDebug(false);
        smutex_.unlock();
        return true;
